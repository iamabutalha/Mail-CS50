...
// List the mail content
    ...
    <button class="btn btn-sm btn-outline-primary mt-2 id="replay" onclick="replay('${email.id}');">Replay</button>
    <button class="btn btn-sm btn-outline-primary mt-2" id="archive" onclick="archive_email(${email.id}, ${email.archived});">${archive}</button>
      <hr>
      <div>${email.body}</div>`;

      // Remove Archive button from emails in Sent email
